...
if (get_session("Cookie_groupID") <> 1 and get_session("Cookie_groupID") <> 2 and get_session("Cookie_koperasiID") <> $koperasiID) {
...
if ($page <> 'list') {
	...
	if ($type == 'vehicle') {
		...
		// '<tr><td>';
		// if ($strStartDate1) {
		// 	print '<input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply" value="Potongan Gaji" class="btn btn-secondary" onclick="PageRefresh();"/>&nbsp;<input type="submit" class="btn btn-secondary" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply_pat" value="Pot. Akuan Tabungan" onclick="PageRefresh();"/>';
		// }
		// '</td></tr>';
	}
...
$strActionPage =
	'?vw=loanJadual78NEW&mn=904&type=' . $type
	. '&page=' . $page
	. '&page_id=1';
...
function PageLink($strPath_, $type_, $page_, $page_id_, $rec_per_page_, $sort_, $id_, $keyword_, $by_, $dept_)
{
	$strTemp_ =
		$strPath_
		. '?vw=loanJadual78NEW&mn=904&type=' . $type_
		. '&page=' . $page_;
