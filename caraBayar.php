...
if ($screen == 'edit') {
    if ($id <> 998) {
        ...
        $strTitles = array('Laman Utama', 'Kemaskini Cara Bayar Anggota');
    }
    ...
    if ($id <> 998) {
        ...
        $strTitles = array('Laman Utama', 'Cara Bayar Anggota');
    }
    $strPages = array($_SERVER['PHP_SELF'], '');
    $align = 'left';
} else {
...
$strTemp .=
    '<div><h5 class="card-title">Kemaskini Cara Bayar Anggota</h5></div>' .
...
if (get_session('Cookie_groupID') <> '') {
    ...
    if ($screen == 'view' or $screen == 'edit') {
        ...
        if ($id == '') {
            $sSQL_ = 'SELECT ID as num FROM syarat';
        ...
        if (isset($_POST['Submit'])) {
            ...
            if ($bPass) {
                $strActivity = $_POST['Submit'] . ' Cara Bayar Anggota';
                activityLog($sSQL, $strActivity, get_session('Cookie_userID'), get_session('Cookie_userName'), 9);
                print '<script>window.location="' . $_SERVER['PHP_SELF'] . '?vw=caraBayar&screen=' . $screen . '&id=' . $id . '";</script>';
                exit;
            }
        }
        ...
        $strTemp .=
            '<form id="content-form" name="MyForm" action="' . $strActionPage . '&vw=caraBayar" method="post">'
            . '<table width="100%" cellping="3" cellspacing="0">';

        if ($screen == 'edit' or $screen == 'view' and $id == '998') {
...
function PageSelection($page_, $strExtra_, $strNameList_, $strPageList_)
{
    ...
    for ($i = 0; $i < count($strNameList_); $i++) {
        ...
        if ($page_ == $strPageList_[$i]) {
            ...
            $strTemp_ .= '<button type="button" class="btn btn-dark"><a href="' . $_SERVER['PHP_SELF'] . '?vw=caraBayar&mn=901&screen=' . $strPageList_[$i];
            $strTemp_ .= '&id=' . $id;
