...
$title     = "Laporan Pecahan Pembiayaan Jubli Emas Pada Bulan ".$mth." Tahun ".$yr.".";
...
/*$sSQL = "SELECT DISTINCT (a.userID),a.name FROM users a, potbulan b, userdetails c, potbulanlook d WHERE b.loanID = d.loanID AND a.userID = b.userID AND a.userID = c.userID AND b.status IN (1) AND c.status IN (1) AND d.loanType IN (2057) AND c.departmentID NOT IN (1598,1600,1599,1584,1706,1841,1770,1664) AND d.yrmth between '".$yymmTT."' and '".$yymm."' ORDER BY CAST(a.userID AS SIGNED INTEGER) DESC";
*/
$sSQL = "SELECT DISTINCT (a.userID) FROM userdetails a, loans b WHERE a.userID = b.userID AND a.status IN (1) AND b.status IN (3) AND b.loanType IN (2057) ORDER BY CAST(a.userID AS SIGNED INTEGER) DESC";
