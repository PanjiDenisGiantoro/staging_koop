 ...
 *          Filename	: 	stockAdjustmentList.php
 *          Date 		: 	04/8/2006
 *********************************************************************************/
...
$sFileName = "?vw=stockAdjustmentList&mn=$mn"; //file name
$sFileRef  = "?vw=stockAdjustment&mn=$mn"; // file ni pergi mane
$title     =  "Pelarasan Stok"; //Title 
...
if ($action == "delete") {
	...
	for ($i = 0; $i < count($pk); $i++) {
		...
		$sWhere = "adjNo=" . tosql($pk[$i], "Text");

		$docNo = dlookup("adjustment", "adjNo", $sWhere);

		$sSQL = "DELETE FROM adjustment WHERE " . $sWhere;
...

$sSQL = "";
$sWhere = "  YEAR(tarikh_adj) = " . $yy;
$sWhere .= " AND addminus = 0";

if ($q <> "" || $debt <> "") {

	if ($by == 2) {
		$sWhere .= " AND adjNo like '%" . $q . "%'";
	}
	// else if ($by == 3) {
...
if ($mm <> "ALL") $sWhere .= " AND MONTH(tarikh_adj) =" . $mm;
$sSQL = $sSQL . $sWhere . ' ORDER BY adjNo DESC';

$getAdj = &$conn->Execute($sSQL);
$getAdj->Move($StartRec - 1);

$TotalRec = $getAdj->RowCount();
$TotalPage =  ($TotalRec / $pg);

$sqlYears = "SELECT DISTINCT YEAR(tarikh_adj) AS year FROM adjustment WHERE tarikh_adj IS NOT NULL AND tarikh_adj != '' AND tarikh_adj != 0 ORDER BY year ASC";
...
if ($by == 2)	print '<option value="2" selected>No Pelarasan</option>';
else print '<option value="2">No Pelarasan</option>';
...
if ($getAdj->RowCount() <> 0) {
	...
	$DRTotal = 0;
	$CRTotal = 0;
	while (!$getAdj->EOF && $cnt <= $pg) {
		$jumlah = 0;


		$tarikh_adj 	= toDate("d/m/y", $getAdj->fields('tarikh_adj'));
		...
		print '
			<td class="Data" style="text-align: left; vertical-align: middle;"><input type="checkbox" class="form-check-input" name="pk[]" value="' . tohtml($getAdj->fields('adjNo')) . '">
			<a href="' . $sFileRef . '&action=view&adjNo=' . tohtml($getAdj->fields('adjNo')) . '&yy=' . $yy . '&mm=' . $mm . '">
			' . $getAdj->fields('adjNo') . '</td>';


		print '
		 <td class="Data" style="text-align: center; vertical-align: middle;">' . $tarikh_adj . '</td>
		 <td class="Data" style="text-align: right; vertical-align: middle;">' . number_format($getAdj->fields('pymtAmt'), 2) . '</td>
	...
	print '
		...
		<tr>
			<td class="textFont">Jumlah Baucer : <b>' . $getAdj->RowCount() . '</b></td>
		</tr>';
} else {
	if ($q == "") {
		print '
