...
*          Filename		: 	selDeptDate.php
*		   Description	:	Selection Date Option with year distinct in transactionacc & Pilihan Jabatan
...
if ($action == "Jana Laporan") {
	...
	if ($msg <> "") {
		...
		$rptURL = $rpt.'.php?dtFrom='.$dtFrom.'&dtTo='.$dtTo.'&kod='.$kod_akaun2.'&field=kod_jabatan';
		print '
		<script>
			var rptUrl;
...
$sSQL = 'SELECT * FROM generalacc WHERE category=\'AH\' ORDER BY code ASC';
...
print '		</select>
		...
		<tr> 
			<td align="right"><b>Pilih Jabatan</b></td>
			<td >'.$strSelect.' 
