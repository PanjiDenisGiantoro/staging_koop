	...
	while (!$GetLoan->EOF && $cnt <= $pg) {
		...
		if ($startPymtDate <> "Proses Baucer") {
			if ($codegroup <> 1638) {
				...
				print '<input type=button value="Cetak" class="btn btn-sm btn-secondary" onClick=window.open("loanJadual78HL.php?type=vehicle&page=view&id=' . $GetLoan->fields(loanID) . '","pop","scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no,");>';
			}
		}
		print '		</td-->
