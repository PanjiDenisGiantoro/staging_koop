...
$sFileName = '?vw=AdvanAdLulus&mn=920';
$sFileRef  = '?vw=AdvanAdLulus&mn=920';
$title     = "Pengurusan Advance Payment Diluluskan";
...
if ($action <> "") { 
	print '	<script>';
	if ($action == "Penyata") {
		print ' rptURL = "AdvanPayYearly.php?yr='.$yr.'&loanID='.$pk[0].'";';
...
print '		</select>&nbsp;&nbsp;           
	...
	if ($GetLoan->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			...
			<input type="button" class="btn btn-sm btn-primary" value="Lejer" onClick="ITRActionButtonClick(\'AdvanPayYearly\');"> &nbsp;';                     
		...
		while (!$GetLoan->EOF && $cnt <= $pg) {
			...
			if($codegroup <> 1638){
				$table  = "?vw=AdvanJadual&mn=920&id=".$GetLoan->fields(loanID);
			}else{
				// $table = "?vw=loanJadual78NEW&mn=906&type=vehicle&page=view&id=".$GetLoan->fields(loanID);
				$table = "?vw=AdvanJadual78Month_Pot&mn=920&type=vehicle&page=view&id=".$GetLoan->fields(loanID);
			}
		...
		print '         <tr class="table-light">
					...
					if ($GetLoan->fields(isApproved) == 1){	
						print ' <input type=button value="Cetak" class="btn btn-sm btn-secondary" onClick="window.location.href=\'?vw=AdvanPayTawaranSah2Tawarruq&ID='.$GetLoan->fields(loanID).'\';"></td>';	
					}
	...
	e = document.MyForm;
		if(e==null) {
			...
			if(count != 1) {
				...
				if (rpt == "AdvanPayYearly" )  {
					url = "AdvanPayYearly.php?pk="+ pk +"&yr='.$yy.'";
				}
