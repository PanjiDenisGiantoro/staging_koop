...
if ($action == 'upload') {
    ...
    if (in_array($file_ext, $allowed_file_types) && ($filesize < 1048576)) {
        ...
        if (file_exists("upload_welfare/" . $newfilename)) {
            // file already exists error
            echo "You have already uploaded this file.";
        } else {
            move_uploaded_file($_FILES["filename"]["tmp_name"], "upload_welfare/" . $newfilename);
            ...
            if (get_session("Cookie_groupID") == 0) {
                if ($pk) {
                    print 'window.location.href="?vw=welfareApply&mn=6&userID=' . $pk . '&picwelfare=' . $newfilename . '&action=view"';
                }
            }
...
<div class="table-responsive">
            ...
            <table class="table mb-3">
                <tr class="table-primary">
                        <td class=Header><h6 class="card-subtitle">Sila Masukkan Dokumen Kebajikan Anda:</h6></td>
                    </tr>
                    <tr class="table-light">
					<td class="Data">';
print '<form action="?vw=uploadwinwelfare&mn=6&action=upload" method=post  enctype="multipart/form-data">
											...
											<center>
											<input type="button" class="btn btn-secondary waves-effect waves-light" value="<<" onClick="window.location.href=\'?vw=welfareApply&mn=6\';">
