...
if (get_session("Cookie_groupID") == 0) {
	...
	$refFileName = '?vw=uploadwinccris&mn=' . $mn . '&action=upload';
} else {
	$userID = dlookup("loans", "userID", "loanID=" . tosql($pk, "Text"));
	$refFileName = '?vw=uploadwinccris&mn=' . $mn . '&pk=' . $pk . '&userID=' . $userID . '&action=upload';
}
	...
	if (in_array($file_ext, $allowed_file_types) && ($filesize < 1048576)) {
		...
		if (file_exists("upload_CCRIS/" . $newfilename)) {
			...
			if (get_session("Cookie_groupID") == 0) {
				...
				print 'window.location.href="?vw=biayaEdit&mn=3&userID=' . $pk . '&pic=' . $newfilename . '&action=view"';
				print '</script>';
			} else {
				$sSQL = "";
				$sWhere = "";
...
print '
	<center>
		<input type="button" class="btn waves-effect waves-light" value="Kembali" style="background-color: white; color: #495057; border: 1px solid gray;" onClick="window.location.href=\'?vw=biayaEdit&mn=3\';">
