...
$title     = 'Muat Naik Lain-Lain';
$max_size = "1048576"; // Max size in BYTES (1MB)

$refFileName = '?vw=uploadwinlainA&mn=' . $mn . '&pk=' . $pk . '&action=upload';
...
if ($action == 'upload') {
    ...
    if (in_array($file_ext, $allowed_file_types) && ($filesize < 1048576)) {
        ...
        if (file_exists("upload_lain/" . $newfilename)) {
            // file already exists error
            echo '<font color="red">Anda telah pun memuat naik fail ini.</font>';
            header("Location: ?vw=uploadwinlainA&mn=$mn&pk=$pk&action=upload");
            exit();
        } else {
            move_uploaded_file($_FILES["filename"]["tmp_name"], "upload_lain/" . $newfilename);
            ...
            print 'window.location.href="?vw=biayaEditA&mn=' . $mn . '&pk=' . $pk . '&pic=' . $newfilename . '&action=view"';
            print '</script>';
        }
...
<div class="file-upload">
    ...
    <div class="mt-3">
        Description :
        <input type="text" lenght="30" size="86" class="form-control" id="desc_lain" name="desc_lain">
    </div>
</div>
<?php
...
print '
	<center>
		<input type="button" class="btn waves-effect waves-light" style="background-color: white; color: #495057; border: 1px solid gray;" value="Kembali" onClick="window.location.href=\'?vw=biayaEditA&mn=' . $mn . '&pk=' . $pk . '\';">
