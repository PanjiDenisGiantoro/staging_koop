...
$sSQL1 = "SELECT * FROM transactionacc WHERE deductID = " . tosql($ID, "Text") . " AND docID IN (15) AND yrmth = " . tosql($yrmth, "Text") . "";
...
if ($Tambah) {
  ...
  $docName = dlookup("generalacc", "code", "ID=" . $ID);

  $strActivity = $_POST['Submit'] . 'Kemasukan Pembuka Akaun Pada ' . $yrmth . ' - ' . $docName;
...
if ($Kemaskini) {
  ...
  $strActivity = $_POST['Submit'] . 'Kemaskini Pembuka Akaun Pada ' . $yrmth . ' - ' . $docName;
  activityLog($sSQL, $strActivity, get_session('Cookie_userID'), get_session('Cookie_userName'), 3);
...
if ($code == 1) {
  ...
  $strActivity = $_POST['Submit'] . ' Hapus Pembuka Akaun - ' . $docName;
  activityLog($sSQLdel, $strActivity, get_session('Cookie_userID'), get_session('Cookie_userName'), 3);

  print '<script>
          alert("Pembuka Carta Akaun Telah Dihapuskan");
