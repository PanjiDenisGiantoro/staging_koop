...
<link href="assets/css/icons.min.css" id="bootstrap-style" rel="stylesheet" type="text/css" />	
</head>';
...
if ($_POST['action'] == 'upload') {
  ...
  if (in_array($file_ext, $allowed_file_types) && ($filesize < 1048576)) {
    ...
    if (file_exists("upload_images/" . $newfilename)) {
      ...
      $sSQL = "";
      $sSQL = "UPDATE userdetails SET picture='" . $newfilename . "' WHERE userID=" . tosql($pk, "Text");
...
print '<h4 class="card-title"><i class="fas fa-upload"></i>&nbsp;' . strtoupper($title) . '</h4>
		...
		<form action="editPic.php?pk=' . $pk . '" method=post  enctype="multipart/form-data">
...
print '
	<center>
		<input type="button" class="btn waves-effect waves-light" value="Kembali" style="background-color: white; color: #495057; border: 1px solid gray;" onClick="window.close()">
