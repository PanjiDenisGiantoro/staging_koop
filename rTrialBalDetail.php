...
$sFileName  = '?vw=rTrialBalDetail&mn='.$mn.'';
$title      = "Laporan Transaksi Penyata Ledger (Trial Balance Detailed)";
...
$rptURL     = 'rptA18style.php?dtFrom='.$dtFrom.'&dtTo='.$dtTo;

$sSQL = "SELECT t.*
FROM transactionacc t
JOIN (
    SELECT docNo, MAX(tarikh_doc) AS latest_tarikh_doc
    FROM transactionacc
    WHERE docID NOT IN (0)
    AND (tarikh_doc BETWEEN '".$dtFrom."' AND '".$dtTo."')
    GROUP BY docNo
) latest ON t.docNo = latest.docNo
WHERE t.docID NOT IN (0)
ORDER BY latest.latest_tarikh_doc DESC, t.docNo, t.tarikh_doc DESC;
";
...
document.getElementById("downloadExcel").addEventListener("click", function() {
        ...
        var dtFrom = "'.$dtFrom.'";
        var dtTo = "'.$dtTo.'";
        var filename = "transaksi_penyata_lejer_" + dtFrom + "_ke_" + dtTo + ".xlsx";
