				...
				if ($_POST) {
					...
					if ($SubmitForm == "Disediakan") {
						...
						$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`, `status`)" .
							" VALUES ('Pembiayaan Disediakan - $loanNo', 'UPDATE', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . $updatedDate . "', '" . $updatedBy . "', '2')";
						...
						$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`, `status`)" .
							" VALUES ('Pembiayaan Disemak - $loanNo', 'UPDATE', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . $updatedDate . "', '" . $updatedBy . "', '2')";
						$rs = &$conn->Execute($sqlAct);
					} elseif ($DisediakanB) {
						$sSQL = "";
						$sWhere = "";
						...
						$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`, `status`)" .
							" VALUES ('Pembiayaan Disahkan - $loanNo', 'UPDATE', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . $updatedDate . "', '" . $updatedBy . "', '2')";
						$rs = &$conn->Execute($sqlAct);
					} elseif ($ajwk2) {
						$updatedBy 	= get_session("Cookie_userName");
						$updatedDate = date("Y-m-d H:i:s");
						...
						if ($lulus) {
							...
							$sSQL .= ", status='" . 3 . "'";
							$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`, `status`)" .
								" VALUES ('Pembiayaan Diluluskan - $loanNo', 'UPDATE', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . $updatedDate . "', '" . $updatedBy . "', '2')";
							$rs = &$conn->Execute($sqlAct);
						}
						if ($tolak) {
							$sSQL .= ", isRejected='" . 1 . "'";
							...
							$sSQL .=  ", status='" . 4 . "'";
							$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`, `status`)" .
								" VALUES ('Pembiayaan Ditolak - $loanNo', 'UPDATE', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . $updatedDate . "', '" . $updatedBy . "', '2')";
							$rs = &$conn->Execute($sqlAct);
						}
						$sSQL = $sSQL . $sWhere;
						if ($debug) print '<br>' . $sSQL;
...
<li><a href="?vw=biayaDokumen&view=AJK&pk=<?= $pk ?>&mn=906" onMouseover="expandcontent('sc2', this)">KELULUSAN JAWATANKUASA</a></li>
				...
				<div class="tab-content p-3" id="myTabContent">
					<div class="tab-pane fade <?php if (@$tabb == 1) {
						...
						<p class="mb-0">
							<?php
							print '<div class="card-header mt-2"><b>Borang Dokumen Proses Pembiayaan</b></div><BR>';
