...
*          Filename		: 	ACCpenghutang.php
*          Date 		: 	06/10/2003
*          Amended 		: 	16/5/2024 - list all syarikat penghutang with their opening balance and due amounts
*********************************************************************************/
include ("common.php");
...
<script language="JavaScript">
	...
	function selSetAnggota(companyID,name,b_Baddress,code,totTunggak,type) 
	{
		...
		window.opener.document.MyForm.invNo.value = "";
...
<table border="0" cellspacing="1" cellpadding="0" width="95%" align="center" class="lineBG">
	<tr>
		...
		<h5 class="card-title">Senarai Syarikat Penghutang</h5>
	...
	if ($GetMember->RowCount() <> 0) {  
		...
		print '	<tr>
				...
				<table border="0" cellpadding="2" cellspacing="1" width="100%" class="table table-sm table-striped" style="font-size: 9pt;">
					...
					<td class="header" align="center"><b>No</b></td>
					...
					while (!$GetMember->EOF && $cnt <= $pg) {
						...
						$sql        		= "SELECT COALESCE(SUM(b_crelim), 0) AS totOpenBalComp FROM generalacc WHERE category = 'AC' AND ID = '$companyID'";
						$rs 				= $conn->Execute($sql);
						$totOpenBalComp		= $rs->fields(totOpenBalComp);	
						if(dlookup("invNo","cb_payments","companyID='" . $companyID ."'") == ''){
							$sqlT			= "SELECT balance 
												FROM cb_payments 
												WHERE invNo = '' 
