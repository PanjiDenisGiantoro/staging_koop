...
*          Filename		: 	selProjDate.php
*		   Description	:	Selection Date Option with year distinct in transactionacc & Pilihan Projek
...
if ($action == "Jana Laporan") {
	...
	if ($msg <> "") {
		...
		$rptURL = $rpt.'.php?dtFrom='.$dtFrom.'&dtTo='.$dtTo.'&kod='.$kod_akaun2.'&field=kod_project';
		print '
		<script>
			var rptUrl;
...
$sSQL = 'SELECT * FROM generalacc WHERE category=\'AI\' ORDER BY code ASC';
...
print '		</select>
		...
		<tr> 
			<td align="right"><b>Pilih Projek</b></td>
			<td >'.$strSelect.' 
