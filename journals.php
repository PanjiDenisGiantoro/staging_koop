...
if($no_jurnal && $action=="view"){
	$sql 			= "SELECT a.*,b.memberID,b.address, b.city, b.postcode, b.stateID, c.name 
						FROM  jurnal a, userdetails b,users c 
...
if($perkara2){
	...
	$bond = isset($no_bond1) ? $no_bond1 : (isset($no_bond) ? $no_bond : '');
	$updatedBy 		= get_session("Cookie_userName");
	$updatedDate 	= date("Y-m-d H:i:s");               
		$userID 	= dlookup("userdetails","userID","memberID = '". $no_anggota . "'");
        if((($perkara2) == 1596) OR (($perkara2) == 1595)) $bond = $userID;
		$deductID 	= $perkara2;
		if($debit2){
		$pymtAmt 	= $debit2;
		$addminus 	= 0;
...
.'<div>&nbsp;</div>'
.'<div class="table-responsive"><form name="MyForm" action="?vw=journals&mn='.$mn.'" method="post">'
...
<tr>
	<td colspan="3">
		...
		if ($jenis == 1)	print '
				<option value="1" selected>KEAHLIAN</option>'; 	else print '
				<option value="1">KEAHLIAN</option>';				
		if ($jenis == 2)	print '
				<option value="2" selected>PINJAMAN</option>'; 	else print '
...
print '
	...
	No. Anggota&nbsp;
		...
		if($action=="new" && $jenis == 1) print '<input type="button" class="btn btn-sm btn-info" value="Pilih" onclick="window.open(\'selToMember.php?refer=f\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
		else if($action=="new" && $jenis == 2) print '<input type="button" class="btn btn-sm btn-info" value="Pilih" onclick="window.open(\'selLoanSAHL.php?refer=f\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
		print '&nbsp;<input name="loan_no" type="hidden" value="">&nbsp;
		&nbsp;<input name="nama_anggota" value="'.$nama.'" type="text" size="40" class="form-control-sm" maxlength="50" readonly/>
	</td>
</tr>
<tr><td>&nbsp;</td></tr>';
//----------
...
print	   '<tr>
				...
				print'
				<input class="form-control-sm" name="no_bond" value="'.$no_bond.'" type="text" size="10" maxlength="50" readonly/>
				</td>';
...
$strSelect = '<select name="'.$name.'" class="form-select-sm" id="deductSelect" onchange="updateDescAkaun(); updateAmount();">
				<option value="">- Kod -';
...
print	   '<tr>
				...
				print'&nbsp;
					<input type="button" class="btn btn-info btn-sm" value="Pilih" onclick="window.open(\'selLoanSAHM.php?refer=f\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">&nbsp;';

					if(count($perkara)<1){
						print'
							<input name="no_bond1" value="'.$no_bond.'" type="text" size="10" maxlength="50" class="form-control-sm" readonly/>';
						}
						else {
							if($no_bond1){
							print'
							<input name="no_bond1" value="'.$no_bond1.'" type="text" size="10" maxlength="50" class="form-control-sm" readonly/>';
						}				
							else{
							print'
							<input name="no_bond1" value=" " type="text" size="10" maxlength="50" class="form-control-sm" readonly/>';
						}				
					}			
				...
				<td class="Data">
					<input name="namagl2" type="text" size="30" maxlength="30" value="'.$keterangan22.'" class="form-control-sm" readonly/>
	...
	<td width="40%" align="right" valign="top">
		<table border="0" cellspacing="1" cellpadding="2">
			...
			<input type="text" name="tarikh_bank" class="form-control-sm" placeholder="dd/mm/yyyy"
				data-provide="datepicker" data-date-container="#tarikh_bank"
				data-date-autoclose="true" value="'.$tarikh_bank.'">
			<div class="input-group-append">
				<span class="input-group-text">
					<i class="mdi mdi-calendar"></i></span>
			</div>
...
<tr>
	...
	<input type="button" name="print" value="Cetakan Koperasi" class="btn btn-secondary" onClick= "print_(\'journalsPaymentPrint.php?id='. $no_jurnal .'\')">&nbsp;
