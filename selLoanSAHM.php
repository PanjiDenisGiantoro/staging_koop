	...
	if ($GetLoan->RowCount() <> 0) {
		...
		while (!$GetLoan->EOF && $cnt <= $pg) {
			...
			print '	<tr>
						...
						<td	class="Data"><a href="javascript:selPinjaman(\'' . $memberID . '\',\'' . $nama_anggota . '\',\'' . $bond . '\');">' . $GetLoan->fields(loanNo) . '-&nbsp;' . dlookup("general", "name", "ID=" . tosql($GetLoan->fields(loanType), "Number")) . '</a></td>
