...
# get file extension of source image file and read it accordingly
	$system=explode(".",$name);
	if ($system[1] == "jpeg" or $system[1] == "jpg" or $system[1] == "JPEG" or $system[1] == "JPG"){$src_img=imagecreatefromjpeg("../picture/".$name);}
...
# get file name, add tn_ and create thumbnail according to $makeimg
	...
	if ($makeimg=="jpg"){
		...
		imagejpeg($dst_img,"thumb_images/".$filename); 
		}
	if ($makeimg=="gif"){
		$filename="tn_".$system[0].".gif";
		imagegif($dst_img,"thumb_images/".$filename); 
		}
	if ($makeimg=="png"){
		$filename="tn_".$system[0].".png";
		imagepng($dst_img,"thumb_images/".$filename); 
		}
