...
if (get_session('Cookie_userID') == "" or get_session("Cookie_koperasiID") <> $koperasiID and get_session("Cookie_groupID") <> 1 and get_session("Cookie_groupID") <> 2) {
	print '<script>alert("' . $errPage . '"); parent.location.href = "index.php";</script>';
}

$sFileName		= "?vw=biayaMohonJaminan&mn=5";
$sActionFileName = "?vw=biayaMember&mn=5";
$sActionFileName2 = "?vw=loanTable&mn=906";
...
if ($SubmitForm <> "") {
	...
	if (count($strErrMsg) == "0") {
		...
		$userID = dlookup("loans", "userID", "penjaminID1 = " . tosql($penjaminID1, "Text") .
			" AND penjaminID2 = " . tosql($penjaminID2, "Text") .
			" AND penjaminID3 = " . tosql($penjaminID3, "Text"));
		...
		$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`, `status`)" .
			" VALUES ('Kemaskini Penjamin - $loanNo', 'UPDATE', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . $updatedDate . "', '" . $updatedBy . "', '2')";
		$rss = &$conn->Execute($sqlAct);

		alert("Maklumat telah dikemaskinikan ke dalam sistem");
		if ((get_session("Cookie_groupID") == 1 or get_session("Cookie_groupID") == 2)) {
			gopage("$sActionFileName2", 1000);
		} else {
			// alert ("Maklumat telah dikemaskinikan ke dalam sistem");
		...
		if ($backToPage <> "") {
			print '<script>window.location.href = "' . $sActionFileName2 . '";</script>';
		}
	} else {
		$SubmitForm = "";
	}
	...
	print '	
		<div >
        	<center>
				...
				<input type="button" name=backToPage class="btn btn-secondary w-md waves-effect waves-light" value="Kembali" onclick="window.location.href=\'?vw=loanTable&mn=906\'">
			</center>
		</div>';
} else {
	print '
