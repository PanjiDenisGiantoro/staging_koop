...
if ($action == 'upload')
{
	...
	{	
		...
		if (file_exists("upload_agm/" . $newfilename))
		{
		...
		{		
			move_uploaded_file($_FILES["filename"]["tmp_name"], "upload_agm/".$newfilename);
...
if (get_session("Cookie_groupID") == 1 OR get_session("Cookie_groupID") == 2) {
	if($pk) {
		print 'window.location.href="?vw=agmApply&mn='.$mn.'&pk='.$pk.'&pic='.$newfilename.'&action=view"';
	}
}
...
<div class="table-responsive">
            ...
            <table class="table mb-3">
                <tr class="table-primary">
                        <td class=Header><h6 class="card-subtitle">Sila Masukkan Dokumen AGM Anda:</h6></td>
                    </tr>
                    <tr class="table-light">
					<td class="Data">';
					
							print '<form action="?vw=uploadwinagm&mn='.$mn.'&action=upload" method=post  enctype="multipart/form-data">
											...
											<center>
											<input type="button" class="btn btn-secondary waves-effect waves-light" value="<<" onClick="window.location.href=\'?vw=agmApply&mn='.$mn.'\';">
