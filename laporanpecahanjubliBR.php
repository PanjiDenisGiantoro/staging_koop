...
$title     = "Laporan Pecahan Pembiayaan Jubli Emas Bersara Pada Bulan ".$mth." Tahun ".$yr.".";
...
$sSQL = "SELECT DISTINCT (a.userID),a.name FROM users a, potbulan b, userdetails c, potbulanlook d WHERE b.loanID = d.loanID AND a.userID = b.userID AND a.userID = c.userID AND b.status IN (2) AND c.status IN (4) AND d.loanType IN (2057) AND c.departmentID NOT IN (1598,1600,1599,1584,1706,1841,1770,1664) AND d.yrmth between '".$yymmTT."' and '".$yymm."' ORDER BY CAST(a.userID AS SIGNED INTEGER) DESC";
...
print ' <table border="0" id="example" cellspacing="1" cellpadding="2" width="100%" class="lineBG" >
		...
		while (!$GetMember->EOF) {
		//TUNAI
		$getJubliBR = getJubliBR($GetMember->fields(userID),$yymmTT,$yymm);
		$jubli = $getJubliBR->fields(bondNo);
		$pokokjubli = $getJubliBR->fields(pokok);
		$untungjubli = $getJubliBR->fields(untung);
print ' <tr>
